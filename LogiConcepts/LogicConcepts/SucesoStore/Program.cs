...
do
{
    ...
    var PDP = GetPorcentajeDepresiacionDelProducto(PA);
...
float GetPorcentajeDepresiacionDelProducto(int PA)
{
    if (PA < 30)
    {
        return 0.95F;
...
decimal GetCostoAlmacenamiento(string? TP, decimal CC, string? TC, decimal PC, int VOL)
{
    if (TP!.Equals("p", StringComparison.CurrentCultureIgnoreCase))
    {
